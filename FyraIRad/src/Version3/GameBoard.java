...
class GameBoard extends JComponent implements Drawable, ActionListener, KeyListener {
	...
	public GameBoard() {
		...
		createWhiteCircles();
		selectedRow = 0;
		currentPlayer = Players.player1;	//Just nu bˆrjar alltid player1 (rˆd)
		winnerDetected = false;
		infoWinnerLocation = new int[4];
	}
	
	private enum STATE {
		MENU, GAME, COMPGAME, HIGHSCORE					//Meny-, tvÂspelare-, enspelare- eller highscorel‰ge
	};
	...
	private void markCircle(int row) {
		...
		if(!arrayOfCircles[row][column].setColor(currentPlayer.getCurrentColor())) {		//markerar crikeln med r√§tt f√§rg
			changeTurn();		//Om en kolumn √§r full √§ndras inga f√§rger och byter turen f√∂r att det inte ska r√§knas som ett drag
			return;				//Avbryter om en kolumn √§r full
		}
	...
	public boolean checkForWinner() {
		return (checkHorizontal() || checkVertically() || 
				checkRightDiagonal() || checkLeftDiagonal());	//Returnerar true om det finns en vinnare n√•gonstans
	}
	...
	public void paint(Graphics g) {
		...
		g.fillRect(10, 10, 700, 600);						//Malar rektangeln som √§r spelplanen
		...
		if (State == STATE.MENU) {					 //Om vi ar i menylaget
			...
			if(winnerDetected) {
				...
				g.fillRect(20 + 100*selectedRow, 20, 80, 580);	//M√•lar ut den rektangeln som visar den aktuellt valda kolumnen
			}
			...
			//H‰r ska highscoren ritas ut pÂ ett bra s‰tt 
	...
	private void paintHighlightWinner(Graphics g, int x1, int y1, int x2, int y2) {
		...
		g2.draw(new Line2D.Float(x1, y1, x2, y2));		//M√•lar rektangel d√§r vinsten √§r
	...
	public void keyPressed(KeyEvent e) {
		...
		/**
		 * Tar hand om alla inmatningar dÂ vi ‰r i meny-l‰get
		 * 
		 */
		...
		/**
		 * Tar hand om alla inmatningar dÂ vi ‰r i tvÂspelarl‰get
		 * 
		 */
		...
		/**
		 * Tar hand om alla inmatningar dÂ vi ‰r i en-spelare-mot-datorn-spell‰get
		 * 
		 */
